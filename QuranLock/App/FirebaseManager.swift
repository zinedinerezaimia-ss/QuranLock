...
// MARK: - Mosque Registration Model
struct RegisteredMosque: Identifiable {
    var id: String
    var name: String
    var city: String
    var project: String
    ...
    init(id: String, data: [String: Any]) {
        ...
        self.collected = data["collected"] as? Int ?? 0
        self.iban = data["iban"] as? String ?? ""
        self.bic = data["bic"] as? String ?? ""
    ...
    var progress: Double {
        ...
        return min(Double(collected) / Double(goal), 1.0)
    }
}
...
class FirebaseManager: NSObject, ObservableObject {
    ...
    @Published var registeredMosques: [RegisteredMosque] = []
    ...
    // MARK: - Mosque Registration

    func registerMosque(name: String, city: String, project: String, goal: Int, iban: String, bic: String, contactEmail: String, completion: @escaping (Bool) -> Void) {
        guard let uid = currentUser?.uid else { completion(false); return }
        ...
        db.collection("mosques").addDocument(data: data) { error in
            DispatchQueue.main.async { completion(error == nil) }
        }
    }
    ...
    func fetchMosques() {
        db.collection("mosques")
            ...
            .getDocuments { [weak self] snapshot, _ in
                let mosques = snapshot?.documents.map {
                    RegisteredMosque(id: $0.documentID, data: $0.data())
                } ?? []
                DispatchQueue.main.async { self?.registeredMosques = mosques }
            }
    }

    func recordDonation(mosqueId: String, amount: Int) {
        let ref = db.collection("mosques").document(mosqueId)
        ref.updateData(["collected": FieldValue.increment(Int64(amount))])

        if let uid = currentUser?.uid {
            db.collection("donations").addDocument(data: [
                "userId": uid,
                "mosqueId": mosqueId,
                "amount": amount,
                "timestamp": FieldValue.serverTimestamp()
...
extension FirebaseManager: ASAuthorizationControllerDelegate {
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        guard
            ...
            DispatchQueue.main.async { self.authError = "Impossible de récupérer les identifiants Apple." }
            return
        }
