...
class QuranService: ObservableObject {
    ...
    @Published var loadingMessage = "Chargement du Coran..."

    private let cacheFileName = "quran_complete_v2.json"
    ...
    private let surahsMeta: [(id: Int, arabic: String, french: String, translit: String, verseCount: Int, type: String)] = [
    ...
    func loadSurah(_ surahId: Int) async {
        ...
        isLoading = true
        loadingMessage = "Chargement de la sourate..."
    ...
    func downloadFullQuran() async {
        isLoading = true
        loadingMessage = "Téléchargement du Coran complet..."
        loadingProgress = 0
        ...
        for (i, surah) in surahs.enumerated() {
            do {
                ...
                loadingProgress = Double(i + 1) / 114.0
                loadingMessage = "Téléchargement... \(i + 1)/114"
            } catch {
                // Skip on error, will retry individually
            }
        }

        saveFullCache(allLoaded)
        isLoading = false
        isOffline = allLoaded.isEmpty
    }

    // MARK: - API Fetch

    private func fetchSurahVerses(surahId: Int) async throws -> [QuranVerse] {
        // Fetch Arabic, French, and phonetic in parallel
        ...
        return zip(zip(arabic.data.ayahs, french.data.ayahs), phonetic.data.ayahs).enumerated().map { i, pair in
    ...
    private func loadFromCache() -> [QuranSurah]? {
        guard FileManager.default.fileExists(atPath: cacheURL.path),
              ...
              let cached = try? JSONDecoder().decode([QuranSurah].self, from: data),
              cached.filter({ $0.isLoaded }).count > 100 else { return nil }
        return cached
    }
